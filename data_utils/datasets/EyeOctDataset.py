...
class EyeOctDataset(Dataset):

    def __init__(self, hypes, train: bool, transforms=None, fold=0):
        super(EyeOctDataset, self).__init__()
        ...
        if fold == 0:
            img_root = os.path.join(root, "images" if train else "images")
            lab_root = os.path.join(root, "masks" if train else "masks")

            img_names = [i for i in os.listdir(img_root)]
    ...
    def fold_dataset_split(hypes):
        ...
        label_list = np.array(
            [os.path.join("masks", f.split('.')[0] + '.png') for f in img_list]
        )
